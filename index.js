...
let final2014 = fifaData.filter(element => element.Year === 2014 && element.Stage === "Final")[0];
...
function getYears(arr, callBack) {
    return callBack(arr).map(element => element.Year);
}
...
function getWinners(arr, callBack) {
    return callBack(arr).map(element => {
...
*/
console.log("Getting to get average goals");
function getAverageGoals(data) {
    let total = data.reduce((sum, current) => {
        return sum += current['Home Team Goals'] + current['Away Team Goals'];
    }, 0);
    console.log(total);
